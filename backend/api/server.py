...
# Add a new endpoint to check Redis status
@app.get("/redis/status")
async def redis_status():
    """Check Redis connection and available data"""
    status = {
    ...
    if status["redis_connected"]:
        for key in ['we', 'others', 'res_ospf', 'wan', 'agg']:
            status["available_dates"][key] = redis_manager.get_available_dates(key)
            status["latest_keys"][key] = redis_manager.get_latest_key(key)
